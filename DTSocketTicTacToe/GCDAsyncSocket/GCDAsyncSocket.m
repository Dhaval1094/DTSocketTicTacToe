               ...
               tag:(long)i
{
	if((self = [super init]))
	{
		...
		else
		{
            ...
            NSData *data = [NSData dataWithData:buffer];
...
- (void)flushSSLBuffers
{
	...
	if (estimatedBytesAvailable > 0)
	{
		...
		do
		{
           // NSLog(@"%lu - estimatedBytesAvailable = %lu", (unsigned long)estimatedBytesAvailable);
...
- (void)doReadData
{
    ...
    if (currentRead == nil) {
        ...
        GCDAsyncReadPacket *tempReadPacket = [[GCDAsyncReadPacket alloc] initWithData:nil
                                                                          startOffset:0
                                                                            maxLength:0
                                                                              timeout:30
                                                                           readLength:8
                                                                           terminator:nil
                                                                                  tag:2];
        currentRead = tempReadPacket;
    }
    
	if (currentRead == nil)
	{
        NSLog(@"GCDCall : Unable to read from  %hu ", self.localPort);
        [self unableToReadAtThisTime];
		return;
	}
    if (flags & kReadsPaused) {
        NSLog(@"GCDCall : Unable to read from  %hu ", self.localPort);
        [self unableToReadAtThisTime];
        return;
    }
